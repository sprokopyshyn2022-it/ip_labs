...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (isEditing && currentEditId) {
      // alert("Завдання 2 - збереження змін (PUT)"); // коментуємо або видаляємо alert
      try {
        ...
        await fetchTodos();   // оновлюємо список
        resetFormState();     // скидаємо форму до початкового стану
      } catch (error) {
  ...
  list.addEventListener("click", async (event) => {
    ...
    if (btn.classList.contains("btn--delete")) {
        ...
        // alert("Завдання 1 - Функція видалення (DELETE)"); // коментуємо або видаляємо alert
        try {
          ...
          // Якщо запит успішний, видаляємо картку з DOM
          card.remove();
          console.log(`Завдання #${taskId} видалено успішно`);
        } catch (error) {
