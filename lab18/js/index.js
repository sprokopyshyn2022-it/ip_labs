document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', (event) => {
        ...
        if (isEditing && currentEditedElement) {
            ...
            const titleEl = currentEditedElement.querySelector('.task-card__title');
            const descEl = currentEditedElement.querySelector('.task-card__desc');
            const tagEl = currentEditedElement.querySelector('.task-card__tag');
            const deadlineEl = currentEditedElement.querySelector('.task-card__deadline');

            if (titleEl) titleEl.textContent = taskData.title;
            if (descEl) descEl.textContent = taskData.description;
            if (tagEl) tagEl.textContent = taskData.tag;

            if (deadlineEl) {
                deadlineEl.textContent = taskData.deadline ? taskData.deadline : 'Без дати';
            }
